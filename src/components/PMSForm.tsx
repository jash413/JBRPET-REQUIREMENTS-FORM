...
const PMSForm: React.FC<PMSFormProps> = ({ onSubmit, formType = 'thermopack' }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Ensure we're using the anon key for this request
      const { data, error } = await supabase
        .from('thermopack_requirements')
        .insert({
          form_data: formData,
          submitter_name: submitterName,
          thermopack_type: formData['thermopack_type'] || null,
          units_to_monitor: formData['units_to_monitor'] || null,
          operating_hours: formData['operating_hours'] || null,
          data_capture_method: formData['data_capture_method'] || null
        })
        .select()
        .single();
      ...
      if (error) {
        ...
        toast({
          ...
          description: `There was an error submitting your form: ${error.message}`,
          variant: "destructive",
        });
        return;
      }
